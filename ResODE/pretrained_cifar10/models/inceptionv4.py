...
class Inception_Stem(nn.Module):
    ...
    def forward(self, x):
        ...
        x = [
            self.branch3x3_conv(x), # nn.sequantial, nn.sequantial
            self.branch3x3_pool(x)
        ]
        x = torch.cat(x, 1) # 96, 64 channels (sum 160)

        x = [
            self.branch7x7a(x), 
            self.branch7x7b(x)
        ] 
        x = torch.cat(x, 1) # 96, 96 channels  (sum 192)
        ...
        x = torch.cat(x, 1) # 192, 192 channels (sum 384 channels)
...
class InceptionV4(nn.Module):
    ...
    def forward_1(self,x): # only stem
        x = self.stem(x)
        return x
        
    def forward_NN(self,x): # except stem
        x = self.inception_a(x)
        x = self.reduction_a(x)
        x = self.inception_b(x)
        x = self.reduction_b(x)

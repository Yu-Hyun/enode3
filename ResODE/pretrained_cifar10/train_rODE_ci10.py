...
class ODEBlock_t(nn.Module):
    ...
    def forward(self, x):
        ...
        out = odeint(self.odefunc, x, self.integration_time, rtol=args.tol, atol=args.tol, method='euler')#method='rk4')#, options=dict(step_size = self.step_size)) #,  options=dict(step_size=5.0) or options=dict(grid_constructor= prtorch 1dim tensor)
        return out[1]

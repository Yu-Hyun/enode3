...
class Eulermodel(nn.Module):
    ...
    def forward(self, x):
        ...
        for idx in range(self.steps):
            x = x +(self.t/self.steps)*(-F.conv_transpose2d(self.relu(self.conv_1(x)),  self.conv_1.weight, stride=1,padding=1,output_padding=0,bias=None) + fx01 + -0.06*x )
        x = x*self.g1[None,:,None,None]
        
        fx02 = self.relu(self.conv0_2(x))
        for idx in range(self.steps):
            x = x +(self.t/self.steps)*(-F.conv_transpose2d(self.relu(self.conv_2(x)),  self.conv_2.weight, stride=1,padding=1,output_padding=0,bias=None) + fx02 + -0.06*x )
...
if __name__ == '__main__':
    ...
    for epoch in range(1, 10 + 1):
        fixed_model_list.append(net_fixed.eval())
        ...
        atk_fgsm_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_pgd_test_avg = pgd(model = net_avg.eval(), steps=20)
        atk_pgd_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_cw_test_avg = CW(model = net_avg.eval())
        atk_cw_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_mifgsm_test_avg = MIFGSM(model = net_avg.eval())
        atk_mifgsm_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_tifgsm_test_avg = TIFGSM(model = net_avg.eval())
        atk_tifgsm_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_difgsm_test_avg = DIFGSM(model = net_avg.eval())
        atk_difgsm_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_nifgsm_test_avg = NIFGSM(model = net_avg.eval())
        atk_nifgsm_test_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        
        
        atk_fgsm_avg = torchattacks.FGSM_avg(fixed_model_list)
        atk_fgsm_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 
        
        atk_pgd_avg = torchattacks.PGD_avg(fixed_model_list)
        atk_pgd_avg.set_normalization_used(mean=[0.5070751592371323, 0.48654887331495095, 0.4409178433670343], std=[0.2673342858792401, 0.2564384629170883, 0.27615047132568404 ]) # cifar100 